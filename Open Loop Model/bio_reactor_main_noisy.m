...
function bio_reactor_main_noisy()
    ...
    deltaD = 0.025 * D0; % Step Change of 2%
    tfinal = 400;
    ...
    %% ===== Save data for FOPTD modeling =====
    save('bio_reactorData_priyam1.mat', ...
         't_vec', ...          % time vector
         'D0', 'deltaD', ...   % base dilution rate and step change
         'y_lin', 'x_nl', ...  % clean linear & nonlinear responses
         'y_lin_noisy', 'x_nl_noisy', ...  % noisy responses
         'x_ss', ...            % steady-state values
         'M', 'B');             % linearized matrices
    disp('Simulation data saved to bio_reactor_data.mat');
end
